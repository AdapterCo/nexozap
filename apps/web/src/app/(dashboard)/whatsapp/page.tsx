...
export default function WhatsAppPage() {
  ...
  const fetchStatus = useCallback(async () => {
    ...
    try {
      const response = await api.get(
        `/companies/${company.id}/whatsapp/status`,
      );
      ...
      setError('Não foi possível buscar o status do WhatsApp');
    }
  }, [company?.id]);
  ...
  const handleConnect = async () => {
    ...
    try {
      const response = await api.post(
        `/companies/${company.id}/whatsapp/connect`,
        {},
        {
          timeout: 30000,
        },
      ...
      const msg =
        err.response?.data?.message || err.message || 'Erro ao conectar';
      setError(msg);
    } finally {
      setLoading(false);
    }
  ...
  const handleDisconnect = async () => {
    ...
    try {
      ...
      const msg =
        err.response?.data?.message || err.message || 'Erro ao desconectar';
      setError(msg);
    } finally {
      setLoading(false);
    }
